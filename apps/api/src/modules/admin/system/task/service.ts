...
export class Service implements OnModuleInit {
  ...
  async initTask(): Promise<void> {
    ...
    // 查找所有需要运行的任务
    const tasks = await prisma.sys_job.findMany({
      where: {
        status: '1',
      },
    });
  ...
  async count(): Promise<number> {
    return await prisma.sys_job.count();
  }
   ...
   * delete task
   */
  async delete(task: sys_job): Promise<void> {
    if (!task) {
   ...
   * 手动执行一次
   */
  async once(task: sys_job): Promise<void | never> {
    if (task) {
      // await this.taskQueue.add(
      //   {
      //     id: task.job_id,
      //     service: task.cron_expression,
      //     args: task.invoke_target,
      //   },
   ...
   * 启动任务
   */
  async start(task: sys_job): Promise<void> {
    if (!task) {
      throw new Error('Task is Empty');
    }
    // 先停掉之前存在的任务
   ...
   * 停止任务
   */
  async stop(task: sys_job): Promise<void> {
    if (!task) {
      throw new Error('Task is Empty');
    }
