...
export class ConfigService {
  static async list(c: Context) {
    ...
    const [total, rows] = await Promise.all([
      db.sys_config.count({ where }),
      db.sys_config.findMany({
        where,
        skip,
        take: pageSize,
  ...
  static async get(c: Context) {
    ...
    const env = c.env as any
    const db = getPrisma(env.DB)
    const data = await db.sys_config.findUnique({ where: { configId: id } })
    
    if (!data) return Result.fail(c, '配置不存在')
  ...
  static async remove(c: Context) {
    ...
    try {
      await db.sys_config.deleteMany({
